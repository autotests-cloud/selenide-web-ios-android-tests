...
@Tag("web")
@Tag("login")
class NegativeLoginTests extends TestBase {

    @Test
    @DisplayName("Negative login in Web app without login. Testid-strategy")
    void negativeLoginWithoutPasswordTest() {
...
//            $(byTestId("Password input")).setValue(DEFAULT_PASSWORD);
    ...
    @Test
    @DisplayName("Negative login in Web app with empty data. Testid-strategy")
    void negativeLoginWithEmptyDataTest() {
        step("Go to login page", ()-> {
            open("http://app.autotests.cloud");
            $(byTestId("Header label")).shouldHave(text("Not authorized"));
        });
